...
router.get('/', async(req, res, next) =>{
    await sql.query('SELECT * FROM artigodemonstracoes;', function (error, results, fields){
        if(error) throw error;
        res.end(JSON.stringify(results))
    });
});
//SELECT o Artigos selecionado ID
router.get('/artigo/:id', (req, res, next) => {
    sql.query('SELECT * FROM artigodemonstracoes where id=?', [req.params.id], function (error, results, fields) {
        if (error) throw error;
        res.end(JSON.stringify(results));
    });
});
...
router.get('/:id', (req, res, next) => {
    sql.query('SELECT * FROM artigodemonstracoes where idDemonstracao=?', [req.params.id], function (error, results, fields) {
        if (error) throw error;
        res.end(JSON.stringify(results));
    });
});
