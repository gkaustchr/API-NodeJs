...
router.get('/', async(req, res, next) =>{
    await sql.query('SELECT * FROM impostos;', function (error, results, fields){
...
router.get('/:id', (req, res, next) => {
    sql.query('select * from impostos where id=?', [req.params.id], function (error, results, fields) {
        if (error) throw error;
        res.end(JSON.stringify(results));
    });
});
...
router.post('/', (req, res) =>{
    var params  = req.body;
    console.log(params);
    sql.query('INSERT INTO regime SET ?', params, function (error, results, fields) {
	  if (error) throw error;
	  res.end(JSON.stringify(results));
	});
});
