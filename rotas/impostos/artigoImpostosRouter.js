...
router.get('/', async(req, res, next) =>{
    await sql.query('SELECT * FROM artigoimpostos;', function (error, results, fields){
        if(error) throw error;
        res.end(JSON.stringify(results))
    });
});
//SELECT artigos pelo id do artigo
router.get('/artigo/:id', (req, res, next) => {
    sql.query('select * from artigoimpostos where id=?', [req.params.id], function (error, results, fields) {
        if (error) throw error;
        res.end(JSON.stringify(results));
    });
});
...
router.get('/:id', (req, res, next) => {
    sql.query('select * from artigoimpostos where idImposto=?', [req.params.id], function (error, results, fields) {
        if (error) throw error;
        res.end(JSON.stringify(results));
    });
});
