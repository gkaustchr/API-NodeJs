...
router.get('/', async(req, res, next) =>{
    await sql.query('SELECT * FROM artigoregime;', function (error, results, fields){
...
router.get('/artigo/:id', (req, res, next) => {
    sql.query('select * from artigoregime where id=?', [req.params.id], function (error, results, fields) {
        if (error) throw error;
        res.end(JSON.stringify(results));
    });
});
...
router.get('/:id', (req, res, next) => {
    sql.query('select * from artigoregime where idRegime=?', [req.params.id], function (error, results, fields) {
        if (error) throw error;
        res.end(JSON.stringify(results));
    });
});
